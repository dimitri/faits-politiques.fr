// Commande inventaire : catalogue JSON de toutes les sources ingérées.
//
...
func main() {
	out := flag.String("out", "docs/catalogue-sources.json", "fichier JSON à écrire")
	racine := flag.String("raw-root", "raw", "racine locale de l'archive scellée")
	flag.Parse()
...
func construire(ctx context.Context, pool *pgxpool.Pool, racine string) (*Catalogue, error) {
	...
	return &Catalogue{
		GenereLe:      nowRFC3339(),
		Note: "Généré par cmd/inventaire — chaque champ vient d'une requête sur raw.*/core.*, " +
