// Command sections-dossiers écrit, dans chaque dossier docs/<dossier>.md, les
...
// crédits par programme (derived.dossier_budget_programme).
		...
		for _, s := range sections {
			...
			if i < 0 || j < i {
				if b.Len() > 0 && !(n == 0 && s.vide != "" && strings.TrimSpace(b.String()) == s.vide) {
					log.Fatalf("%s : marqueurs %s absents alors que la base a du contenu pour cette section", chemin, s.code)
				}
				continue
			}
