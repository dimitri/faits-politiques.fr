...
func IngestPensionsEIR(ctx context.Context, pool *pgxpool.Pool, arch *archive.Archive) error {
	...
	if _, err := tx.Exec(ctx, `DELETE FROM core.pension_tranche_eir WHERE annee = $1`, annee); err != nil {
		return fail(err)
	}
	if _, err := tx.CopyFrom(ctx, pgx.Identifier{"core", "pension_tranche_eir"},
	...
	arch.EndRun(ctx, runID, "SUCCESS", map[string]any{"tranches": len(rows), "total_pct": total}, "")
	fmt.Printf("  distribution des pensions (EIR %d) : %d tranches, total %.1f %%\n", annee, len(rows), total)
	return nil
}
