...
func run(ctx context.Context, only, rawDir, migDir string) error {
	...
	// Personnels non enseignants par catégorie précise (direction, CPE,
	// AESH...) — voir internal/education/personnel_categorie.go. Hors
	// chaîne par défaut.
	if only == "education-personnel-categorie" {
		fmt.Println("\néducation nationale : personnels non enseignants par catégorie")
	...
	if only == "eau-aides-rhin-meuse" {
		fmt.Println("\naides de l'agence de l'eau Rhin-Meuse")
		return eau.IngestAidesRhinMeuse(ctx, pool, arch)
	}
	// Budget annexe M49/M49A (eau, assainissement), communes et EPCI — voir
	// internal/eau/budget_annexe.go. Hors chaîne par défaut.
	if only == "eau-budget-annexe" {
		fmt.Println("\nbudget annexe eau/assainissement (OFGL, M49/M49A)")
